...
/**
 * Detects if we're running inside a Discord Activity iframe.
 * Discord adds frame_id + instance_id query params to the URL.
 */
function isRunningInDiscordActivity() {
    try {
        ...
        const discordReferrer = referrer.includes('discord.com') || referrer.includes('discordsays.com');
        return (hasFrameId || hasInstanceId) || (inIframe && discordReferrer);
    } catch (e) {
        return false;
    }
}

let discordActivitySDK = null;
let discordActivityAuth = null;
let isDiscordActivity = false;
let _activityInitStarted = false; // Guard against double-init

/**
 * Initializes the Discord Embedded App SDK.
 * IMPORTANT: This runs FULLY independently from the main app loading flow.
 * Errors here NEVER affect page rendering.
 */
async function initDiscordActivity() {
    // Guard: only run once
    if (_activityInitStarted) return;
    _activityInitStarted = true;

    // Check if SDK loaded and we're in Discord
    if (!window.DiscordSDK) {
        console.log('[Discord Activity] SDK not available, skipping.');
        return;
    }

    isDiscordActivity = isRunningInDiscordActivity();
    if (!isDiscordActivity) {
        console.log('[Discord Activity] Not inside Discord iframe, skipping.');
        return;
    }

    console.log('[Discord Activity] Running inside Discord! Initializing...');

    const clientId = (document.body && document.body.getAttribute('data-discord-client-id')) || '';
    if (!clientId || clientId === 'YOUR_DISCORD_APPLICATION_CLIENT_ID') {
        console.error('[Discord Activity] Client ID not set! Edit data-discord-client-id on <body> in index.html.');
        return;
    }

    try {
        discordActivitySDK = new window.DiscordSDK(clientId);

        // Step 1: SDK handshake with Discord client
        await discordActivitySDK.ready();
        console.log('[Discord Activity] SDK ready!');

        // Step 2: Authorize (Discord shows consent if needed)
        ...
        console.log('[Discord Activity] Got authorization code.');

        // Step 3: Exchange code for access_token via backend
        // NOTE: these endpoints must be deployed to Heroku first!
        const tokenResponse = await fetch(`${CHAT_API_BASE}/discord/activity-token`, {
            method: 'POST',
        ...
        if (!access_token) { console.warn('[Discord Activity] No access_token from backend.'); return; }

        // Step 4: Authenticate with Discord client (required to use SDK features)
        discordActivityAuth = await discordActivitySDK.commands.authenticate({ access_token });
        if (!discordActivityAuth) { console.warn('[Discord Activity] Authenticate failed.'); return; }
        console.log('[Discord Activity] Authenticated as:', discordActivityAuth.user?.username);

        // Step 5: Create a session in our chat backend
        const sessionResponse = await fetch(`${CHAT_API_BASE}/discord/activity-login`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (sessionResponse.ok) {
            ...
            if (sessionData.session_token) {
                localStorage.setItem('RadioGaming-discordAuthToken', sessionData.session_token);
                discordAuthToken = sessionData.session_token;
                discordUser = sessionData.user;
                updateAuthUI(true);
                initializeChatPolling();
                showNotification('Logged in via Discord Activity!', 'fab fa-discord');
                console.log('[Discord Activity] Chat session active for', discordUser.username);
                // Hide login button — already logged in via Activity
        ...
        console.error('[Discord Activity] Non-fatal init error:', error.message);
        // Page renders normally regardless of this error
    }
}

// Attempt Activity init AFTER the page has fully rendered (setTimeout 0 = next event loop tick)
// This ensures Activity init NEVER blocks or delays the main app render.
document.addEventListener('DOMContentLoaded', () => {
    setTimeout(() => {
        if (window.DiscordSDK) {
            initDiscordActivity();
        } else {
            // SDK module might load after DOMContentLoaded — listen for it
            document.addEventListener('discord-sdk-ready', () => initDiscordActivity(), { once: true });
        }
    }, 0);
});
